...
const sendMoney = async (req, res) => {
  try {
    ...
    const { amount, accountType, accountID, currency, receivingCurrency, description, agentPhoneNumber } = req.body;
    ...
    if (currency !== receivingCurrency) {
      ...
      if (!exchangeRate) {
        return res.status(400).json({ status: 400, message: 'Unsupported currency conversion' });
      }
    ...
    const transaction = await Transaction.create({
      userId, amount, accountType, accountID, currency, receivingCurrency, amountReceived, description, agentPhoneNumber, transactionDate: new Date()
    });
