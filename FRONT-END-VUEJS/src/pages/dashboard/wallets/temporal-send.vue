...
const addRecipient = () => {
  const filled = Object.values(newRecipient.value).every(v => v);
  if (!filled) return;
        ...
        <div class="flex flex-col gap-10 px-8 pb-12 pt-8">
          ...
          <span v-if="sendData.step === 1" class="flex flex-col gap-10">
            <button
              ...
              :key="option"
              @click="sendData.type = option"
              :class="{ 'bg-primary/5': sendData.type === option }"
              class="flex items-center gap-x-4 p-6 border rounded-lg"
            >
              <HomeIcon />
              <div>
                <p class="font-semibold">{{ option === 'bank' ? 'Direct Bank' : 'Mobile Money' }}</p>
                <span class="text-xs">Transfer to {{ option === 'bank' ? 'bank' : 'mobile money' }}</span>
              </div>
